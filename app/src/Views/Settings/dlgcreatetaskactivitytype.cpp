...
void Views::DlgCreateTaskActivityType::prepareView()
{
    ...
    m_btnOk->clicked().connect(this, &Views::DlgCreateTaskActivityType::btnOkClicked);

    m_btnCancel = new Wt::WPushButton("Cancel", this->footer());
    m_btnCancel->clicked().connect(this, &Wt::WDialog::reject);
}

bool Views::DlgCreateTaskActivityType::validate()
{
...
void Views::DlgCreateTaskActivityType::btnOkClicked()
{
    if(validate())
        this->accept();
}
